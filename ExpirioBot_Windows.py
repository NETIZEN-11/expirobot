#!/usr/bin/env python3
"""
ExpirioBot - Windows Version with OCR
Main entry point for Windows with camera and Tesseract OCR (mock arm).
"""
...
class ExpirioBotWindows:
    """Main application class for Windows version with OCR."""
    ...
    def initialize(self) -> bool:
        ...
        logger.info("Initializing ExpirioBot (Windows with OCR)...")
        ...
        if not self.ocr_processor.is_available:
            ...
            logger.info("Alternative: Run ExpirioBot_NoTesseract.py for manual date entry")
            return False
        ...
        if not self.camera.start():
            logger.warning("Camera not available - GUI will start but camera features won't work")
            # Don't return False, allow GUI to run without camera
        else:
            logger.info("Camera initialized")
        
        # Initialize GUI
        self.gui = ExpirioBotGUI("ExpirioBot Control Panel - Windows (OCR Mode)")
        self.gui.setup_video_display()
        self.gui.setup_counters()
        self.gui.setup_control_buttons()
        self.gui.setup_warning("⚠️ Running in MOCK MODE (No physical arm) - OCR Enabled")
...
def main():
    """Main entry point."""
    print("=" * 60)
    print("🤖 ExpirioBot - Windows Version (OCR Mode)")
    print("=" * 60)
    print("Hardware: Camera (Mock Arm)")
    print("OCR: Tesseract")
    print("=" * 60)
    
    app = ExpirioBotWindows()
    app.run()
